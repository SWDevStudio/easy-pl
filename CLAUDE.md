...
Дочерние таблицы события (`event_signups`, `event_slots`, `attendance`) своего
`uid` не имеют — они синхронизируются пакетом вместе с событием и версионируются
его `updated_at`. **Любая функция, трогающая дочернюю таблицу, заканчивается
`touchEvent(eventId)`** — без этого заявки и явка не уедут вообще: сборщик
изменений отбирает события по `updated_at`, и неизменившаяся отметка означает
