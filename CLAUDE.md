...
### Синхронизация: у записи есть `uid`, у удаления — надгробие

Локальные `id` автоинкрементные и на разных машинах столкнутся, поэтому всё, что
синхронизируется (`classes`, `raids`, `players`, `events`), несёт `uid`.

- **Новая запись получает `uid` в репозитории при вставке** (`newUid()` из
  [src/db/uid.ts](src/db/uid.ts)). Забыли — запись просто не уедет на сервер.
- **Удаление пишет надгробие** через `recordTombstone()` перед `DELETE`. Без него
  удаление на одной машине воскреснет при следующей синхронизации с другой.
- **Любая правка синхронизируемой таблицы обновляет `updated_at`** — по нему
  собираются изменения и разрешаются конфликты (побеждает более поздняя правка).
- У сид-данных `uid` выводится из естественного ключа, а не случайный: две свежие
  установки должны получить одинаковые идентификаторы для одного и того же класса,
  иначе справочник задвоится.

Дочерние таблицы события (`event_signups`, `event_slots`, `event_quotas`,
`attendance`) своего `uid` не имеют — они синхронизируются пакетом вместе с
событием и версионируются его `updated_at`. **Любая функция, трогающая дочернюю
таблицу, заканчивается `touchEvent(eventId)`** — без этого заявки и явка не уедут
вообще: сборщик изменений отбирает события по `updated_at`, и неизменившаяся
отметка означает «тут ничего не поменялось». У квот эту роль выполняет
`refreshTotalSlots`: он же пересчитывает `events.slots` и ставит отметку.

Ссылки внутри пакета едут по `uid`, а не по локальному `id` — у квоты это
`raidUid`. `raidUid: null` значит «без рейда», а неизвестный uid откладывает
событие (`deferred`), как и неизвестный игрок. Пустой массив квот и **отсутствие
поля** `quotas` — разные вещи: первое значит «мест не выделено», второе — правка
от клиента старой версии, и тогда локальные квоты не трогаются, а событию без
квот выдаётся одна строка «без рейда» на всё `slots`.

**Отметка времени — всегда полный ISO-8601 с `Z`** (`new Date().toISOString()`).
Голая дата `2026-08-09` короче и в лексикографическом сравнении встаёт раньше
любой отметки того же дня, а сервер такие значения отклоняет целиком — один
кривой формат заваливает весь обмен.

**Сид получает фиксированную отметку времени, а не момент установки.** Иначе
свежая установка выглядит «новее» всех и откатывает правки справочника на всех
машинах — установка, которая не трогала ни одного класса, стирает чужие
переименования.

**Обмен идёт сам, а не по кнопке.** Правки уезжают при запуске, через 30 секунд
тишины после последней записи в базу и при закрытии окна. «Грязь» ловится в
одном месте — обёртка над `execute()` в [client.ts](src/db/client.ts) смотрит на
запрос и дёргает подписчиков; звать `touch()` из каждого стора не нужно, и забыть
негде. На время применения чужих правок отслеживание глушится
(`withoutTracking`), иначе обмен запускал бы сам себя по кругу.

**Фоновый обмен молчит при сбоях.** Ошибку показываем только когда человек нажал
кнопку сам; отвалившийся VPN превращается в состояние в шапке, а не во всплывашку
поверх работы. Спиннеры фоновый режим тоже не трогает: `isBusy` — про ручные
действия, `isSyncing` — про фон.
...
значит перешагнуть через чужие записи, закоммиченные в ту же секунду, и больше
никогда их не запросить.

### Секреты живут на сервере, а не на компьютерах

`SYNC_TOKEN` и `DISCORD_TOKEN` — секреты Cloudflare Worker. На машинах хранится
только ключ синхронизации (в диспетчере учётных данных Windows), и приходит он
внутри кода подключения `EASYPL1-…`, который человек вставляет один раз.

Запросы к Discord идут **через наш Worker**, а не из приложения: токен бота
общий на гильдию, и раздавать его каждому — значит раздать полный доступ к боту.
Worker умеет ровно шесть чтений, поэтому утёкший код подключения не даёт
ничего сверх них. Заодно из webview всё равно нельзя ходить в Discord напрямую —
CORS.

Обращения к серверу идут одной командой `sync_request(url, path, payload)`:
Rust добавляет `Authorization: Bearer` и возвращает тело как есть. Разбор ответа
и текст ошибок — на стороне Worker'а, чтобы сообщение было одно, а не три разных
в трёх слоях.

**Привилегированные интенты Discord включаются руками в Developer Portal** и
ссылкой-приглашением не запрашиваются. Без `SERVER MEMBERS INTENT` список
участников не отдаётся; без `MESSAGE CONTENT INTENT` текст сообщений приходит
пустым **без ошибки** — HTTP 200 и пустая строка вместо текста.

### Места на осаде считаются по рейдам, а не одним числом

У события нет «числа слотов» как единственной величины: места выделяются каждому
рейду отдельно (`event_quotas`), и группа «без рейда» — такой же рейд со своей
квотой. `events.slots` — производная сумма квот, её поддерживает
`refreshTotalSlots`, а не пользовательский ввод.

- **Недобор рейда не перераспределяется.** Не хватило желающих — места остаются
  пустыми, их закроет пати-лидер. Любая логика вида «раздадим остаток тем, кто
  ещё хочет» ломает смысл: состав рейда собирает его лидер.
- **`share` для долга берётся из квоты своего рейда**, а не из события. Общий
  `events.share` остаётся только как запас для клиентов старой версии.
- Розыгрыш ведёт отдельный поток случайных чисел на группу (`seed:raid_id`),
  иначе изменение состава одного рейда сдвигает жребий соседнего.
- «В состав» после жеребьёвки проверяет квоту **того рейда, в котором игрок**.

**Обязательных больше, чем мест — это предупреждение, а не отказ.** Жеребьёвка
не должна упираться в «снимите «точно идёт» с лишних»: сорванная кнопка посреди
сбора состава дороже, чем лишнее место. Окно подтверждения показывает рейд и
числа и даёт три выхода — вывести сверх квоты (`seatPriorityOverQuota` в
`draw()` поднимает `slots` группы до числа обязательных), вывести игрока в
рейде со свободными местами или снять флаг. Сама `draw()` без флага по-прежнему
падает `PriorityOverflowError`: молча раздувать квоту нельзя, это решение
человека.

**Вывод в чужом рейде — разовый, `players.raid_id` он не трогает.** Рейд на одну
осаду живёт в `event_signups.raid_id` (`NULL` — «в своём рейде»), едет в пакете
события по `uid` и умирает вместе с заявкой. Менять справочник ради одной осады
нельзя: игрок вернулся бы в чужой рейд навсегда, а прошлые осады пересчитались
бы задним числом — они группируются по тому же полю.

Отсюда правило: **группа игрока на осаде — это `COALESCE(s.raid_id, p.raid_id,
0)`, а не `p.raid_id`.** Любой новый запрос, который считает места, заявки,
явку или долю рейда, обязан join'ить `event_signups`; забыли — разово выведенный
попадёт в свою группу, и квоты разъедутся с составом. Удаление рейда сбрасывает
эти отметки (`dropRaidFromEvents`), иначе останется группа с мёртвой ссылкой.
...
**Иконка не переembedится сама.** `tauri-build` зашивает `icons/icon.ico` в
ресурсы exe из build-скрипта, а перезапуск скрипта Cargo отслеживает по
`tauri.conf.json`, не по самим файлам иконок. Подменили иконки, конфиг не
трогали — соберётся старая, молча и без единого предупреждения. После
`tauri icon` обязателен `cargo clean -p easy-pl`.

**Закрытие окна требует отдельного разрешения.** `core:default` не включает
`core:window:allow-destroy`, и без него обработчик `onCloseRequested`, вызвавший
`preventDefault()`, вешает окно навсегда: `destroy()` отклоняется, до закрытия
дело не доходит, а пользователь видит просто неработающий крестик. Разрешение
прописано в [capabilities/default.json](src-tauri/capabilities/default.json)
рядом с `sql:allow-execute` — и по той же причине.

Обработчик закрытия обязан быть неубиваемым: перехват срабатывает один раз
(флаг `closing`), ожидание синхронизации ограничено таймаутом, а `destroy()`
подстрахован `close()`. Любая ошибка внутри не должна оставлять окно открытым.
...
### Уведомления — только через `useToast`

Плашки «сохранено» и «не вышло» живут в одной очереди —
[useToast](src/composables/useToast.ts). Состояние вынесено на уровень модуля,
поэтому все, кто позвал composable, пишут в общий список, а рисует его один
`UiToaster` в `App.vue`. Свои `ref`-ы под уведомления в компонентах не заводим.

```ts
const toast = useToast();

toast.success("Данные сохранены");
toast.error("Сервер отклонил ключ");
toast.show({ text: "Идёт обмен", class: "alert-info", timeout: 0 });
```

Цвет приходит классом (`success` и `error` — тонкие обёртки над `show`), а не
пропом-перечислением: компонент склеивает его через `cn`, как и всё остальное.

**Фоновые сбои тостами не показываем.** Уведомление — про действие, которое
человек только что сделал. Отвалившийся VPN во время автообмена уходит в
состояние в шапке; тост об ошибке всплывает, только если нажали кнопку сами.
